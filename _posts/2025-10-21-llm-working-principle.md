...
Tensor의 성질들은 Transformer의 다양한 연산을 정확하게 수행하는데 필수적인 요소들입니다. Tensor가 어떻게 구성되고 처리되는지를 이해하는 것이 모델의 디버깅, 최적화, 커스터마이징에 큰 도움이 됩니다.
...
Weighted Sum은 단순히 정보를 삭제하지 않고, 모든 벡터의 선형 결합을 만듭니다. 따라서 입력 벡터의 정보를 **비율 조정만 해서 보존**합니다.
