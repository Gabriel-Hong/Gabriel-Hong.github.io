...
다행히 GEN NX는 270여 개의 REST API를 제공합니다. 이걸 LLM이 직접 호출하게 만들면 자연어 → 구조모델 → 해석까지의 흐름이 성립합니다.
...
"3경간 연속보 + 자중 + 고유치해석"이 노드·요소·재료·경계·하중·해석 순서로 풀려 나가는 것은 세 가지의 **조합**입니다.
...
셋 중 하나라도 빠지면 — `instructions`를 비우거나, tool 이름을 `api_7f3b_create`처럼 불투명하게 짓거나 — 분해 정확도가 떨어집니다. MCP 서버 품질의 상당 부분이 서버가 책임지는 두 축(instructions, tool 메타데이터)의 품질에 달려 있습니다.

### 4.5 tool 수와 정확도

매 턴 tool 메타데이터 전체가 컨텍스트에 들어가므로, tool을 많이 노출하면 두 가지 문제가 생깁니다.
...
65개의 JSON 스키마 파일이 런타임에 46개의 엔드포인트로 인덱싱되고, 각 엔드포인트에서 HTTP method마다 독립된 MCP tool이 생성되어 최대 **159개의 tool**이 노출됩니다. codegen 단계는 없습니다. JSON 파일만 추가/수정하고 서버를 재기동하면 끝입니다.
...
파일명이 아니라 `endpoint` 필드로 그룹핑하는 이유는, GEN NX 스키마에서 하나의 논리적 엔드포인트가 여러 파일로 쪼개져 있는 경우가 있기 때문입니다.

엔드포인트 하나에 여러 JSON 파일이 묶였을 때, GEN NX 스키마에는 두 가지 패턴이 혼재합니다.
...
"하나의 함수가 method를 인자로 받는" 방식이 아니라 method별로 시그니처가 다릅니다. LLM이 시그니처만으로 의도를 파악하게 하기 위함입니다.
...
`_build_assign_schema()`는 원본 JSON Schema를 LLM 친화적 parameter schema로 변환합니다. 5가지 case 분기입니다:
...
JSON Schema만으로는 LLM이 필드 구조는 알지만 실제 값에서 자주 틀립니다. 예제 한 줄을 붙이면 올바른 payload 생성 비율이 크게 올라갑니다.

### 5.4 tool 등록 — 일반적인 MCP 패턴과 다른 점

MCP tool을 등록하는 일반적인 방법부터 살펴보겠습니다.
...
여기서 끝입니다. `Annotated[dict, Field(...)]`로는:
...
이런 정보를 `inputSchema`에 넣을 수 없습니다. `Field(description=...)`은 flat한 텍스트 하나를 받을 뿐이고, JSON Schema의 `additionalProperties` 안에 중첩된 `properties`를 Pydantic 모델로 일일이 정의하는 것은 46개 엔드포인트마다 다른 필드 구조를 가진 상황에서 비현실적입니다.

거기에 스케일 문제가 겹칩니다. 46개 엔드포인트 x 최대 4개 HTTP method = **159개 tool**의 시그니처를 수동으로 정의하는 건 Atlassian MCP의 40개와 차원이 다릅니다.

#### GEN NX의 해법 — parameters 오버라이드

ToolFactory는 `FunctionTool.from_function()`으로 tool을 만든 뒤, **`tool.parameters`를 JSON Schema 기반으로 덮어쓰는** 방식을 씁니다.
...
`Annotated[dict, Field(description="...")]`이었다면 `{"type": "object"}`로 끝났을 내용이, 오버라이드를 통해 **필드별 type/description + 예제 payload + sub-type 정보**까지 포함한 풍부한 schema가 됩니다.
...
반면 `doc/*` API들은 `Assign` 패턴을 따르지 않습니다 — 파라미터가 없거나(`doc/NEW`, `doc/SAVE`), 문자열 하나(`doc/OPEN`의 `file_path: str`), 또는 선택적 dict(`doc/ANAL`의 `Argument: dict | None`)입니다. 파라미터가 단순하므로 일반적인 데코레이터 패턴이 오히려 적합합니다.
...
#### tool-level description — 한영 병기

`inputSchema`와 별개로, LLM이 tool을 **선택**할 때 보는 tool-level description은 `utils/descriptions.py`의 `generate_description()`이 생성합니다.
...
결과적으로 `post_db_node`의 description은 `"Create nodes (절점) - define geometry points in 3D space in GEN NX. Provide data in the Assign parameter."` 형태가 됩니다. 한글 도메인 용어가 포함되어 있어서, 한국어 요청에서도 tool 선택 정확도가 올라갑니다.
...
태그만 올바르게 붙여두면 새 엔드포인트 추가 시 필터링 로직을 건드릴 필요가 없습니다. JSON 파일 추가 + sub-server의 `ENDPOINTS` dict에 한 줄 등록이면 끝입니다.
...
- instructions 구성 고도화 — tool 간 의존관계, 도메인 용어, 자주 틀리는 패턴 등을 보강해서 MCP 동작 정확도를 높이는 방향
